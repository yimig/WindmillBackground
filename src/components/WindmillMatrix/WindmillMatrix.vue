...
export default {
  ...
  methods:{
    ...
    pattern_OnMouseOver(event){
      d3.select(event.target.parentElement).select('animateTransform').attr('repeatCount','indefinite')
    },
    pattern_OnMouseOut(event){
      d3.select(event.target.parentElement).select('animateTransform').attr('repeatCount',1)
    }
  },
  mounted() {
    this.initMatrix()
